...
export default function SuperAdminPage() {
  ...
  return (
    <div style={{...styles.container, padding: isMobile ? '20px' : '40px'}}>
      <h1 style={{...styles.title, fontSize: isMobile ? '24px' : '32px'}}>Painel Super Admin</h1>
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={{...styles.inputGroup, marginTop: '15px', flexDirection: isMobile ? 'column' : 'row'}}>
          ...
          <div style={{display: 'flex', gap: '10px', marginTop: '15px', flexDirection: isMobile ? 'column' : 'row'}}>
            <button type="submit" style={{...styles.button, width: isMobile ? '100%' : 'auto'}}>{editingId ? 'Salvar Alterações' : 'Criar Barbearia'}</button>
            {editingId && <button type="button" onClick={() => {setEditingId(null); setFormData({ name: '', slug: '', phone: '', email: '', password: '' });}} style={{...styles.cancelButton, width: isMobile ? '100%' : 'auto'}}>Cancelar</button>}
          </div>
        </form>
                  ...
                  <tr key={t.id} style={styles.tr}>
                    ...
                    <td style={styles.td}>
                      <div style={{display: 'flex', gap: '10px', flexWrap: 'wrap'}}>
                        <a href={`/${t.slug}`} target="_blank" style={{...styles.linkButton, padding: isMobile ? '8px 12px' : '5px 10px', fontSize: isMobile ? '12px' : '14px'}}>Ver Site</a>
                        <button onClick={() => handleEdit(t)} style={{...styles.editButton, padding: isMobile ? '8px 12px' : '5px 10px', fontSize: isMobile ? '12px' : '14px'}}>Editar</button>
                        <button onClick={() => handleDelete(t.id)} style={{...styles.deleteButton, padding: isMobile ? '8px 12px' : '5px 10px', fontSize: isMobile ? '12px' : '14px'}}>Excluir</button>
                      </div>
                    </td>
                  </tr>
                ))}
...
const styles = {
  container: { backgroundColor: '#0A0A0A', minHeight: '100vh', color: '#fff', fontFamily: 'Arial' },
  title: { marginBottom: '30px', color: '#8205ff', fontWeight: 'bold' },
  ...
  linkButton: { color: '#8205ff', textDecoration: 'none', fontWeight: 'bold', border: '1px solid #8205ff', padding: '5px 10px', borderRadius: '5px', textAlign: 'center' },
